...
const SubService: NextPage<SubServiceProps> = ({ layout, meta }) => {
  return (
    <>
...
export const getStaticPaths: GetStaticPaths = async (ctx) => {
  ...
  const response = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_API}/api/services-pages?${query}`
  )
  const paths = response?.data
    .map((item: any) => {
      ...
      if (parent)
        return {
          params: { service: parent, subservice: item.attributes.slug },
        }
    })
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const parentSlug = data.parent.data.attributes.slug
  if (parentSlug !== params?.service) {
    return {
      notFound: true,
    }
  }
