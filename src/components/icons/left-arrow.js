import React from 'react'

const LeftArrow = (props) => {
  return (
    <svg
      width='1em'
      height='1em'
      {...props}
      version='1.1'
      xmlns='http://www.w3.org/2000/svg'
      viewBox='0 0 47.255 47.255'
      style={{ enableBackground: 'new 0 0 47.255 47.255' }}
    >
      <g>
        <path
          fill='currentColor'
          d='M34.941,47.255c-0.256,0-0.512-0.098-0.707-0.293L11.607,24.334c-0.391-0.391-0.391-1.023,0-1.414L34.234,0.293
		c0.391-0.391,1.023-0.391,1.414,0s0.391,1.023,0,1.414l-21.92,21.92l21.92,21.92c0.391,0.391,0.391,1.023,0,1.414
		C35.453,47.157,35.197,47.255,34.941,47.255z'
        />
      </g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
    </svg>
  )
}

export default LeftArrow
